import { BarChart3, CreditCard, CalendarDays, Percent } from 'lucide-react'
...
export function StatsCards({ metrics }: { metrics: HostMetrics }) {
    return (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mb-8">
            ...
            <div className="bg-white dark:bg-zinc-900 overflow-hidden rounded-xl border border-gray-200 dark:border-zinc-800 p-6 shadow-sm">
                <div className="flex items-center gap-4">
                    <div className="p-3 bg-purple-100 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400 rounded-lg">
                        <BarChart3 className="h-6 w-6" />
                    </div>
                    <div>
                        <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Precio Medio / Noche</p>
                        <h3 className="text-2xl font-bold text-gray-900 dark:text-white">
                            {new Intl.NumberFormat('es-ES', { style: 'currency', currency: 'EUR' }).format(metrics.averageNightlyRate / 100)}
                        </h3>
                    </div>
                </div>
            </div>
